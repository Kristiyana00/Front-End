(() => {
	...
	firebase.auth().onAuthStateChanged(user => {
		...
		if (validateUser()) {
			...
			window.auth.getUserStats(user.uid).on('value', (data) => {
				const {
					tweets,
					likes
				} = data.val() || {tweets:0, likes: 0};
	...
	tweetsDB.on('child_added', data => {
		...
		if(!validateUser())
		{
			...
			new_post.innerHTML = post(data);
			postContainer.prepend(new_post);

			new_post.querySelector('.like-btn').addEventListener("click", event => {
				tweet.incrementLikes(event.target.getAttribute('data-id'));
				event.preventDefault();
			})

			new_post.querySelector('.dislike-btn').addEventListener("click", event => {
				tweet.incrementDislikes(event.target.getAttribute('data-id'));
				event.preventDefault();
			})

			new_post.querySelector('.post-close').addEventListener('click', event => {
				const toDelete = event.target.getAttribute('data-id');
				tweet.delete(toDelete);
				document.getElementsById('post-container').removeChild(event.target.parentNode.parentNode);
				event.preventDefault();
			})
	});
	...
	tweetsDB.on('child_changed', data => {
		...
		let likes2 = document.querySelector(`.like-btn[data-id=${data.key}]`);
		likes2.nextElementSibling.innerHTML = data.val().likes;
		let dislikes2 = document.querySelector(`.dislike-btn[data-id=${data.key}]`);
		dislikes2.nextElementSibling.innerHTML = data.val().dislikes;
		window.auth.getUserStats(firebase.auth().currentUser.uid).once('value').then(data => {
			const {
				likes,
				tweets
			} = data.val();
